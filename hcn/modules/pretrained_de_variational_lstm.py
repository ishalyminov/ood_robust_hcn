...
class PretrainedDEVariationalHierarchicalLSTM(LSTMBase):
    def __init__(self, in_rev_vocab, in_config, in_feature_vector_size, in_action_size, in_ae_model_folder):
        self.rev_vocab = in_rev_vocab
        self.ae_model_folder = in_ae_model_folder

        super(PretrainedDEVariationalHierarchicalLSTM, self).__init__(in_config, in_feature_vector_size, in_action_size)
        if self.ae_model_folder:
            saver = tf.train.Saver(var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='CompatibleRNNAutoencoder'))
            saver.restore(self.sess, os.path.join(self.ae_model_folder, ae_ood.CompatibleRNNAutoencoder.MODEL_NAME)) 

    def __graph__(self):
        ae = ae_ood.CompatibleRNNAutoencoder(self.config, self.rev_vocab)
        with tf.variable_scope('PretrainedDEVariationalHierarchicalLSTM'):
            ...
            vae_turn_encodings = vrae.decoder_init_state
            ae_turn_encodings = ae.enc_state
            ae_reconstruction_loss = ae.loss_op

            turn_features = tf.concat([tf.reshape(vae_turn_encodings, shape=[-1, self.max_input_length, self.config['rnn_size']]),
                                       tf.reshape(ae_turn_encodings, shape=[-1, self.max_input_length, self.config['rnn_size'] * 2]),
                                       input_contexts],
                                      axis=-1)

            # input projection
            Wi = tf.get_variable('Wi',
                                 shape=[self.feature_vector_size + self.nb_hidden * 3, self.nb_hidden],
                                 dtype=tf.float32,
                                 initializer=xav())
            ...
            outputs_with_loss = tf.concat([outputs,
                                           tf.reshape(ae_reconstruction_loss, shape=[-1, self.max_input_length, 1])],
                                          axis=-1)
            # output projection
            Wo = tf.get_variable('Wo',
                                 shape=[self.nb_hidden + 1, self.action_size],
                                 dtype=tf.float32,
                                 initializer=xav())
            bo = tf.get_variable('bo',
                                 ...
                                 initializer=tf.constant_initializer(0.))
            # get logits
            logits = tf.tensordot(outputs_with_loss, Wo, axes=1) + bo
            # probabilities
            ...
            self.vae_nll_loss = vrae._nll_loss_fn()
            ...
            filtered_gradients, filtered_variables = gradients, variables  # [], []
            #for grad, var in zip(gradients, variables):
            #    if 'CompatibleRNNAutoencoder' in var.name:
            #        continue
            #    filtered_gradients.append(grad)
            #    filtered_variables.append(var)
            gradients, _ = tf.clip_by_global_norm(filtered_gradients, self.config['clip_norm'])
        self.train_op = optimizer.apply_gradients(zip(filtered_gradients, filtered_variables), global_step=self.global_step)
    ...
    def forward(self, enc_inp, dec_inp, dec_out, bow_out, context_features, action_mask, y):
        ...
        probs, prediction, loss, hcn_loss, vae_nll_loss, vae_kl_loss, vae_bow_loss = self.sess.run(
            [self.probs, self.prediction, self.loss, self.hcn_loss, self.vae_nll_loss, self.vae_kl_loss,
             self.vae_bow_loss],
            feed_dict={self.vrae.enc_inp: enc_inp_flattened,
                       self.vrae.dec_inp: dec_inp_flattened,
                       self.vrae.dec_out: dec_out_flattened,
                       self.vrae.bow_targets: bow_out_flattened,
                       self.vrae.sequence_length_for_decoder: dec_seq_length,
                       self.ae.enc_input: enc_inp_flattened,
                       self.ae.dec_output: dec_out_flattened,
        ...
        return prediction, {'loss': np.ones_like(vae_nll_loss) * loss,
                            'hcn_loss': np.ones_like(vae_nll_loss) * hcn_loss,
                            'vae_nll_loss': vae_nll_loss,
    ...
    def train_step(self, enc_inp, dec_inp, dec_out, bow_out, context_features, action_mask, y):
        ...
        _, loss, hcn_loss, vae_nll_loss, vae_kl_loss, vae_bow_loss, lr = self.sess.run(
            [self.train_op, self.loss, self.hcn_loss, self.vae_nll_loss, self.vae_kl_loss, self.vae_bow_loss, self.lr],
            feed_dict={self.vrae.enc_inp: enc_inp_flattened,
                       self.vrae.dec_inp: dec_inp_flattened,
                       self.vrae.dec_out: dec_out_flattened,
                       self.vrae.bow_targets: bow_out_flattened,
                       ...
                       self.action_mask_: action_mask})
        # maintain state
        return ({'loss': np.ones_like(vae_nll_loss) * loss,
                 'hcn_loss': np.ones_like(vae_nll_loss) * hcn_loss,
                 'vae_nll_loss': vae_nll_loss,
                 'vae_kl_loss': vae_kl_loss,
                 'vae_bow_loss': vae_bow_loss},

...
class MaskedDualEncoderVariationalHierarchicalLSTM(LSTMBase):
    def __init__(self, in_rev_vocab, in_config, in_feature_vector_size, in_action_size):
        self.rev_vocab = in_rev_vocab
        super(MaskedDualEncoderVariationalHierarchicalLSTM, self).__init__(in_config, in_feature_vector_size, in_action_size) 

    def __graph__(self):
        with tf.variable_scope('vae'):
            vrae = getattr(vae, self.config['vae_model'])(self.config, self.rev_vocab, self.sess)
        with tf.variable_scope('ae'):
            ae = ae_ood.MaskedRNNAutoencoder(self.config, self.rev_vocab)
        ...
        turn_features = tf.concat([tf.reshape(vae_turn_encodings, shape=[-1, self.max_input_length, self.config['rnn_size']]),
                                   tf.reshape(ae_turn_encodings, shape=[-1, self.max_input_length, self.config['embedding_size'] * 2]),
                                   input_contexts],
                                  axis=-1)
        ...
        loss = tf.reduce_mean(self.hcn_loss + self.vae_overall_loss + self.ae_loss)
    ...
    def forward(self, enc_inp, dec_inp, dec_out, bow_out, context_features, action_mask, y):
        ...
        decoder_output_mask = np.ones((dec_out_flattened.shape[0]))
        probs, prediction, loss, hcn_loss, vae_nll_loss, vae_kl_loss, vae_bow_loss = self.sess.run([self.probs, self.prediction, self.loss, self.hcn_loss, self.vae_nll_loss, self.vae_kl_loss, self.vae_bow_loss],
                                                                                                   feed_dict={self.vrae.enc_inp: enc_inp_flattened,
                                                                                                              ...
                                                                                                              self.ae.decoder_output_mask: decoder_output_mask,
    ...
    # training
    def train_step(self, enc_inp, dec_inp, dec_out, bow_out, context_features, action_mask, decoder_output_mask, y):
        enc_inp_flattened = enc_inp.reshape(enc_inp.shape[0] * enc_inp.shape[1], enc_inp.shape[-1])
        dec_inp_flattened = dec_inp.reshape(dec_inp.shape[0] * dec_inp.shape[1], dec_inp.shape[-1])
        dec_out_flattened = dec_out.reshape(dec_out.shape[0] * dec_out.shape[1], dec_out.shape[-1])
        bow_out_flattened = bow_out.reshape(bow_out.shape[0] * bow_out.shape[1], bow_out.shape[-1])
        ...
        _, loss, hcn_loss, vae_nll_loss, vae_kl_loss, vae_bow_loss, lr = self.sess.run([self.train_op, self.loss, self.hcn_loss, self.vae_nll_loss, self.vae_kl_loss, self.vae_bow_loss, self.lr],
                                        feed_dict={self.vrae.enc_inp: enc_inp_flattened,
                                                   self.vrae.dec_inp: dec_inp_flattened,
                                                   self.vrae.dec_out: dec_out_flattened,
                                                   self.vrae.bow_targets: bow_out_flattened,

...
from modules.masked_dual_encoder_variational_hierarchical_lstm import MaskedDualEncoderVariationalHierarchicalLSTM
...
from utils.preprocessing import (make_vocabulary,
                                 ...
                                 generate_random_input_for_variational_hcn,
                                 PAD,
                                 START,
                                 UNK,
                                 EOS,
...
class Trainer(object):
    def __init__(self, in_train, in_dev, in_test, in_action_templates, in_random_input, in_rev_vocab, in_config, in_model_folder):
        ...
        self.net = MaskedDualEncoderVariationalHierarchicalLSTM(in_rev_vocab, self.config, context_features.shape[-1], action_masks.shape[-1])
    ...
    def train(self):
        ...
        for j in range(epochs):
            ...
            for batch in batch_gen:
                ...
                decoder_output_mask = np.ones((dec_out[0].shape[0]))
                for idx in range(num_turns):
                    if np.random.random() < random_input_prob:
                        decoder_output_mask[idx] = 0
                        ...
                        decoder_output_mask[idx] = 0 
                batch_loss_dict, lr = self.net.train_step(enc_inp, dec_inp, dec_out, bow_out, context_features, action_masks, decoder_output_mask, y)
            # evaluate every epoch
            train_accuracy, train_mean_losses = evaluate(self.net, self.data_train, runs_number=3)
            train_loss_report = ' '.join(['{}: {:.3f}'.format(key, value) for key, value in train_mean_losses.items()])
            dev_accuracy, dev_mean_losses = evaluate(self.net, self.data_dev, runs_number=3)
...
def main(in_dataset_folder, in_custom_vocab_file, in_model_folder, in_config):
    ...
    random_input = generate_random_input_for_variational_hcn(10000, config['max_sequence_length'], vocab, rev_vocab)

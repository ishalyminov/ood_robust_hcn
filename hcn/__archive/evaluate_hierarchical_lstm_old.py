...
def evaluate_advanced(in_model, in_dataset, in_action_templates, in_dialogs, ignore_ood_accuracy=False):
    ...
    for y_true, y_pred in zip(y_true_dialog, y_pred_dialog):
        ...
        for y_i_true, y_i_pred in zip(y_true, y_pred):
            # print('{}\t{} {} {}'.format(turn_counter, in_dialogs[turn_counter][0], in_dialogs[turn_counter][1], '(!)' if prev_action == backoff_action and y_i_true != backoff_action else ''))
            ...
            if prev_action == backoff_action and y_i_true != backoff_action:
                ...
                post_ood_turns.append(turn_counter)
            if ood_occurred:
    ...
    return {'avg_loss': np.mean(losses),
            ...
            'total_post_ood_turns': total_post_ood_turns,
            'correct_post_ood_turns': correct_post_ood_turns}
...
def main(in_dataset_folder, in_model_folder, in_no_ood_evaluation):
    ...
    X, context_features, action_masks, y = make_dataset_for_hierarchical_lstm(test_dialogs,
                                                                              test_indices,
                                                                              vocab,
                                                                              et,
                                                                              at,
    ...
    net = HierarchicalLSTM(config, context_features.shape[-1], action_masks.shape[-1])
    net.restore(in_model_folder)
    eval_stats_full_dataset = evaluate_advanced(net,
                                                (X, context_features, action_masks, y),
                                                test_dialogs,
                                                at.action_templates)

...
class VRAE(object):
    ...
    def _build_graph(self):
        with tf.variable_scope('VRAE', reuse=tf.AUTO_REUSE):
    ...
    def _encode(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            if self.config.get('bi_lstm'):
                forward_encoder = self._rnn_cell(self.config['rnn_size'] // 2)
                backward_encoder = self._rnn_cell(self.config['rnn_size'] // 2)
                ...
                _, encoded_state = tf.nn.dynamic_rnn(cell=self._rnn_cell(self.config['rnn_size']),
                                                     inputs=encoder_emb,
                                                     sequence_length=self.enc_seq_len,
                                                     dtype=tf.float32)
    ...
    def _decoder_training(self, z):
        ...
        with tf.variable_scope('decoding'):
            self.decoder_init_state = tf.layers.dense(self.z, self.config['rnn_size'], tf.nn.elu)
    ...
    def _decoder_inference(self, z):
        ...
        with tf.variable_scope('decoding', reuse=True):
            init_state = tf.layers.dense(self.z, self.config['rnn_size'], tf.nn.elu, reuse=True)
    ...
    def load(in_model_folder, in_session):
        ...
        ae = VRAE(config, rev_vocab, in_session, standalone=True)
